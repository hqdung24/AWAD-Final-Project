...
- **Scheduling & payments**: Cleanup + reminder crons run every 5 minutes. Cleanup expires stale locks/payments/bookings; reminders send 24h/3h trip emails and respect notification preferences. Trip status auto-updates every minute with real `in_progress`; trips archive 1 day after arrival; route deactivation worker drains every 30s.
