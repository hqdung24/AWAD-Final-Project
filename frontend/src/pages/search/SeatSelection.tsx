...
import { getSeatsByTrip, lockSeats, type SeatStatus } from '@/services/seatService';
...
export default function SeatSelection() {
  ...
  const { data: seatStatuses, isLoading: seatsLoading, error: seatsError, refetch: refetchSeats } = useQuery({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const now = Date.now();
      const remaining = Math.floor((lockedUntil.getTime() - now) / 1000);
  ...
  const handleSeatClick = (seatId: string, state: string) => {
    ...
    if (state !== 'available') return; // Can't select non-available seats
  ...
  const handleContinueToCheckout = async () => {
    ...
    try {
      ...
      notify.success('Seats locked successfully! Redirecting to checkout...');

      // Navigate to checkout page with lock_token and seat_ids
      navigate(`/search/${id}/checkout?from=${from}&to=${to}&date=${date}&passengers=${passengers}&lock_token=${response.lock_token}&seat_ids=${selectedSeats.join(',')}`);
    } catch (error) {
  ...
  const getSeatColor = (state: string) => {
    switch (state) {
      ...
      case 'locked':
        return 'bg-orange-100 text-orange-700 border-orange-300 cursor-not-allowed';
      case 'available':
      default:
        ...
        <div className="container mx-auto px-4 py-8">
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">Loading seat information...</h3>
        ...
        <div className="container mx-auto px-4 py-8">
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">Seat information not available</h3>
              <p className="text-muted-foreground mb-4">Unable to load seat data for this trip</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <Button
            variant="ghost"
            onClick={() => navigate(`/search/${id}?from=${from}&to=${to}&date=${date}&passengers=${passengers}`)}
          >
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Trip Details
          </Button>
          
          {lockToken && timeRemaining && (
            <div className="flex items-center gap-2 px-4 py-2 bg-orange-100 text-orange-700 rounded-lg border border-orange-300">
              <Clock className="h-4 w-4" />
              <span className="font-semibold">Time remaining: {formatTimeRemaining(timeRemaining)}</span>
            </div>
          )}
        </div>
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="text-2xl">
              Trip Details: {trip.from} → {trip.to} | {trip.company} | {formatTime(trip.departureTime)} - {formatTime(trip.arrivalTime)}
            </CardTitle>
          </CardHeader>
        </Card>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {seatRows.map(([rowLetter, seats]) => {
                      ...
                      return (
                        <div key={rowLetter} className="flex justify-center gap-8">
                            ...
                            {leftSeats.map((seatStatus) => {
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() => handleSeatClick(seatStatus.seatId, seatStatus.state)}
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{formatPrice(trip.price * selectedSeats.length)}</span>
                  </div>
                ...
                <Button
                  ...
                  onClick={lockToken 
                    ? () => navigate(`/search/${id}/checkout?from=${from}&to=${to}&date=${date}&passengers=${passengers}&lock_token=${lockToken}&seat_ids=${selectedSeats.join(',')}`)
                ...
                {lockToken && (
                  <div className="pt-4 border-t">
                    <div className="flex items-start gap-2 text-sm text-orange-700 bg-orange-50 p-3 rounded-lg">
                      ...
                      <p>
                        Your seats are reserved. Complete your booking within {timeRemaining && formatTimeRemaining(timeRemaining)} or they will be released.
                      </p>
                    </div>
                  </div>
                )}
